create or replace function public.claim_modusign_signature_request(
  p_pledge_id uuid,
...
returns public.signature_documents
language plpgsql
security definer
set search_path = public
as $$
declare
  current_pledge public.pledges;
  claimed_document public.signature_documents;
begin
  select *
    into current_pledge
    from public.pledges
  ...
  if current_pledge.id is null or current_pledge.donor_user_id <> p_donor_user_id then
    raise exception 'pledge_not_found';
  end if;

  if current_pledge.status <> 'draft' then
    raise exception 'pledge_state_changed';
  end if;

  if current_pledge.version <> p_expected_version then
    raise exception 'pledge_version_changed';
  end if;
...
revoke all on function public.claim_modusign_signature_request(uuid, uuid, integer, text, text, timestamptz) from public;
grant execute on function public.claim_modusign_signature_request(uuid, uuid, integer, text, text, timestamptz) to service_role;
...
begin
  ...
  update public.pledges
   ...
   where id = target_pledge_id
     and status in ('draft', 'awaiting_donor_signature');
