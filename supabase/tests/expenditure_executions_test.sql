...
select ok(
  not has_function_privilege(
    'authenticated',
    'public.register_expenditure_execution(uuid,uuid,uuid,jsonb,jsonb,text)',
    'EXECUTE'
  ),
...
select is(
  (select plan_item_id from public.expenditure_executions where idempotency_key = 'execution-submit-key-0001'),
  '8aaaaaaa-2000-4000-8000-000000000002'::uuid,
  'the reviewed plan item is reassociated atomically'
);
select is(
  (select semantic_key from public.expenditure_executions where idempotency_key = 'execution-submit-key-0001'),
